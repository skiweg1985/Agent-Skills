...
## Agent personas on `coder2`

These names identify the worker behind the shared GitHub account and must be used for Git commit attribution and GitHub signatures unless the current orchestrator explicitly supplies a different mapping:

- OpenAI Codex → `Axel Foundry`
- Claude Code → `Claudia Koi`
- OpenCode, regardless of the selected OpenRouter model → `Mike`

Before any Linear write, verify that the client's OAuth/MCP identity matches the intended persona. A model family or provider name such as GPT, Claude, Gemini, or OpenRouter is not an agent persona. Every dispatched run must also receive a stable session identifier and owning issue identifier; stop before repository-host writes when either is missing.
